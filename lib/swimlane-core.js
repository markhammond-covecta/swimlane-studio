  ...
  for (let lineNo = 0; lineNo < lines.length; lineNo++) {
    ...
    if ((m = line.match(arrowRe))) {
      ...
      if (/^<-{1,2}>$/.test(arrow)) {
        ...
        lanesSeen.add(from);
        lanesSeen.add(to);
        events.push({ type: "message", from, to, text, dashed, bidirectional: true, arrowLabel, tag: lineTag, lineNo });
        continue;
      }
    ...
    if ((m = line.match(incomingOnlyRe))) {
      ...
      if (lane && text) {
        lanesSeen.add(lane);
        events.push({ type: "message", from: lane, to: lane, text, incomingOnly: true, arrowLabel, tag: lineTag, lineNo });
        continue;
      }
    }
...
function render(model) {
  ...
  boxes.forEach(box => {
    ...
    if (box.arrowPathIn && box.arrowPathIn.length >= 2) {
      conns.push({ pts: box.arrowPathIn, radius: 8, dash: "", dataLine: box.lineNo });
    }
