  ...
  for (let lineNo = 0; lineNo < lines.length; lineNo++) {
    ...
    if ((m = line.match(arrowRe))) {
      ...
      if (/^<-{1,2}>$/.test(arrow)) {
        ...
        lanesSeen.add(from);
        lanesSeen.add(to);
        // A dashed "<-->" dashes the link ARRIVING at this box, not the
        // forward trail leaving it: dashing the arrow into a box is the whole
        // point of the form (a plain "-->" already dashes the outgoing one),
        // and its caption sits on that same incoming arrow.
        events.push({ type: "message", from, to, text, dashed: false, dashedIn: dashed, bidirectional: true, arrowLabel, tag: lineTag, lineNo });
        continue;
      }
...
// null if the input has no trailing <...> token. The caption may already
// contain real line breaks (unescapeLabel runs first), so match across
// them: "." would stop at the first "\n" and leave the <label> in the box.
...
function solveLayout(model) {
  ...
  for (const ev of model.events) {
    ...
    // A caption that rides the INCOMING arrow (see the dashed-in forms) sits
    // in the gap BEFORE this box, between it and the previous box in the
    // lane, so the gap to widen is that one rather than the outgoing one.
    const onIncoming = ev.incomingOnly || ev.dashedIn;
    let baseCol, targetCol;
    if (onIncoming) {
      let prevCol = null;
      for (const p of placedBoxes) {
        if (p.col >= ev.col) continue;
        if (p.lane === fromI || (p.lanes && p.lanes.includes(fromI))) {
          if (prevCol === null || p.col > prevCol) prevCol = p.col;
        }
      }
      if (prevCol === null) continue; // no incoming arrow is drawn
      baseCol = prevCol;
      targetCol = ev.col;
    } else {
      ...
      baseCol = ev.col;
      targetCol = nextCol !== null ? nextCol : (ev.col + 1);
    }

    // Wrap the caption and find the widest line (canvas-measured). A caption
    // drawn beside a VERTICAL arrow wraps at the tighter width, matching what
    // arrowLabelSvg does, so the two agree on how much room is needed.
    const wrapAt = (model.orientation === "vertical") ? L.VLABEL_MAX_CHARS : L.HLABEL_MAX_CHARS;
    const lines = wrap(ev.arrowLabel, wrapAt);
    let widest = 0;
    ...
    let defaultGap = GAP;
    for (let c = baseCol + 1; c < targetCol; c++) defaultGap += (colExtent[c] || defaultExt) + GAP;
    if (needed > defaultGap) {
      const extra = needed - defaultGap;
      // Apply to baseCol's right margin. With this model, each unit of
      // right margin widens the gap by exactly one unit (boxes at
      // col baseCol stay put; cols after baseCol shift right by `extra`).
      colRightMargin[baseCol] = Math.max(colRightMargin[baseCol] || 0, extra);
    }
  }
...
function render(model) {
  ...
  model.lanes.forEach((lane, i) => {
    ...
    // data-lane-band marks the whole band as a hit target for lane drag-to-
    // reorder (the caption text carries data-lane for double-click rename).
    parts.push(`<rect x="${frameX}" y="${y}" width="${frameW}" height="${laneH}" rx="10" ry="10" fill="#fafbfc" stroke="#d6dae0" stroke-width="1.25" data-lane-band="${i}"/>`);
...
function arrowLabelSvg(arrowPath, label, side, centerY, isVertical, lineNo) {
  ...
  const fontSize = 11;
  const lineH = 13;
  // Wrap by how the arrow reads in the FINAL svg: the diagram's vertical
  // orientation transposes the solver frame, so a solver-horizontal segment
  // is drawn as a vertical arrow (whose caption sits in a narrow side gap and
  // wants the tighter wrap) and vice versa.
  const drawnHorizontal = isVertical ? !isHorizontal : isHorizontal;
  const maxChars = drawnHorizontal ? LAYOUT.HLABEL_MAX_CHARS : LAYOUT.VLABEL_MAX_CHARS;
  ...
  if (isVertical) {
    ...
    if (isHorizontal) {
      // Final: vertical arrow. The label goes BESIDE it, so it must clear the
      // line rather than straddle it: anchor the text's near edge a gap away
      // from the arrow (solver y maps to final x) and let it run outward.
      // Anchoring at the midpoint with only a few px of offset would overlap
      // the line, since the text is centred on its anchor.
      const useSide = side || "right";
      const sideGap = 8;
      x = midX;
      y = useSide === "left" ? midY + sideGap : midY - sideGap;
      anchor = useSide === "left" ? "start" : "end";
      baseline = "middle";
    } else {
      ...
      //
      // A counter-pair shares a column: the solver puts the first arrow at
      // -offset (drawn above) with side "left" and the second at +offset
      // (drawn below) with side "right". Each caption goes on the OUTSIDE
      // of its own line, so the second one hangs below rather than landing
      // in the gap between the two arrows.
      belowArrow = side === "right";
      x = belowArrow ? midX + aboveGap : midX - aboveGap;
      y = (typeof centerY === "number") ? centerY : midY;
      anchor = "middle";
      baseline = belowArrow ? "hanging" : "auto";
    }
  ...
  // ABOVE the arrow line so the bottom of the text clears the arrow; stack
  // them downward from the anchor when it hangs BELOW; otherwise centre the
  // lines symmetrically around the anchor.
  const aboveArrow = !belowArrow && (isHorizontal || (isVertical && !isHorizontal));
  const startDy = aboveArrow
    ? -(lines.length - 1) * lineH
    : belowArrow ? 0 : -(lines.length - 1) * lineH / 2;
...
// Rewrite the source so the lanes render in `order`. An existing "order:"
// directive is rewritten in place (keeping its indentation and casing);
// otherwise one is inserted after "title:" if there is one, else at the very
// top. Lane names are written verbatim, so a name containing a comma cannot
// round-trip — such a name is left to the first-seen ordering by returning
// the source untouched.
function reorderLanes(source, order) {
  const names = (order || []).map(n => String(n).trim()).filter(Boolean);
  if (!names.length) return source;
  if (names.some(n => n.includes(","))) return source;

  const lines = String(source).split("\n");
  const directive = names.join(", ");
  for (let i = 0; i < lines.length; i++) {
    const raw = lines[i];
    const body = raw.trim();
    if (!body || body.startsWith("#") || body.startsWith("//")) continue;
    const m = raw.match(/^(\s*)(order)(\s*:\s*)(.*)$/i);
    if (m) {
      lines[i] = m[1] + m[2] + (m[3].includes(" ") ? m[3] : ": ") + directive;
      return lines.join("\n");
    }
  }

  // No directive yet: insert one. Keep it under an existing "title:" so the
  // title stays the first line of the document.
  let at = 0;
  for (let i = 0; i < lines.length; i++) {
    const body = lines[i].trim();
    if (!body || body.startsWith("#") || body.startsWith("//")) continue;
    if (/^title\s*:/i.test(body)) at = i + 1;
    break;
  }
  lines.splice(at, 0, "order: " + directive);
  return lines.join("\n");
}
